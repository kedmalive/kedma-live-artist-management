...
const ArtistCard: React.FC<ArtistCardProps> = ({ artist, onClick }) => (
  <div 
    className="group relative h-[450px] sm:h-[500px] md:h-[550px] w-full rounded-xl sm:rounded-2xl overflow-hidden cursor-pointer shadow-2xl transition-all duration-700 hover:-translate-y-4 hover:shadow-amber-500/40 bg-[#0a0a0a] border border-white/5"
  ...
  >
    ...
    {artist.credit && (
      <div className="absolute top-4 sm:top-6 left-4 sm:left-6 z-20 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0">
        <span className="bg-black/60 backdrop-blur-md text-[9px] sm:text-[10px] text-white/90 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full flex items-center gap-1 sm:gap-1.5 border border-white/10 font-bold uppercase tracking-wider">
          <Camera size={10} className="sm:w-3 sm:h-3 text-amber-500" />
          {artist.credit}
        </span>
      </div>
    )}
    ...
    <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 md:p-8 z-30 translate-y-2 group-hover:translate-y-0 transition-all duration-500">
      <div className="flex items-center gap-2 mb-3 sm:mb-4">
        <div className="w-8 sm:w-10 h-[2px] bg-amber-500" />
        <span className="text-amber-500 text-[10px] sm:text-[11px] font-black tracking-[0.15em] sm:tracking-[0.2em] uppercase">
          {artist.englishName}
        </span>
      </div>
      
      <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-2 sm:mb-3 group-hover:text-amber-500 transition-colors tracking-tighter leading-none uppercase italic">
        {artist.name}
      </h3>
      ...
      <div className="max-h-0 group-hover:max-h-40 overflow-hidden transition-all duration-700 ease-in-out opacity-0 group-hover:opacity-100">
        ...
        <div className="flex items-center text-black bg-amber-500 text-xs font-black gap-2 group-hover:gap-4 transition-all uppercase tracking-tighter w-fit px-4 sm:px-6 py-2 sm:py-3 rounded-full shadow-2xl hover:bg-white">
