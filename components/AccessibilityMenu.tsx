...
const AccessibilityMenu: React.FC<AccessibilityMenuProps> = ({ 
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMenu}
        className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-[90] bg-amber-500 text-black p-3 sm:p-4 rounded-full shadow-2xl hover:scale-110 transition-transform focus:outline-none focus:ring-4 focus:ring-amber-300"
      ...
      {isOpen && (
        <div className="fixed bottom-20 sm:bottom-24 right-4 sm:right-6 z-[90] w-[calc(100vw-2rem)] sm:w-80 max-w-sm bg-white text-black rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-100 overflow-hidden animate-fade-in-up">
          <div className="p-4 sm:p-6 bg-amber-500 text-black flex justify-between items-center">
          ...
          <div className="p-4 sm:p-6 space-y-5 sm:space-y-6" dir="rtl">
            ...
            >
              ...
              <div className={`w-10 h-6 rounded-full relative transition-colors ${accessibilitySettings.highContrast ? 'bg-amber-500' : 'bg-gray-300'}`}>
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${
                accessibilitySettings.underlineLinks 
                  ? 'bg-amber-50 text-amber-900 border-2 border-amber-500/20' 
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >
              <div className="flex items-center gap-3">
              ...
              <div className={`w-10 h-6 rounded-full relative transition-colors ${accessibilitySettings.underlineLinks ? 'bg-amber-500' : 'bg-gray-300'}`}>
